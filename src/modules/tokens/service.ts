...
/** Single-use: called when the task behind the link is completed. */
...
/**
 * Standard completion for an external task: task → done, token burned,
 * audit entry. Every magic-link action ends here (except multi-visit
 * flows like the employer wizard, which burn the token only when complete).
 */
